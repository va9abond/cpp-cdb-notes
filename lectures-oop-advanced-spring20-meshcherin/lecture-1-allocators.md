...
> [!TIP]
  ...
  Во время вызова оператора `new[]` компилятор запоминает количество
  элементов, память для которых он выделил: при вызове `new[n]`
  он запрашивает чуть больше памяти для того, чтобы дополнительно
  сохранить количество элементов
  `n`, и после он помнит об этом количестве, когда вызывается оператор
  `delete[]`
